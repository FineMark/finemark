...
pub(crate) fn block_document_parser(parser_input: &mut ParserInput) -> Result<Vec<Element>> {
    ...
    while !parser_input.input.is_empty() {
        ...
        match element_parser(parser_input) {
...
fn parse_line_block(parser_input: &mut ParserInput) -> Result<Option<Element>> {
    match parser_input.state.block_mode {
        BlockMode::FullDocument | BlockMode::NestedDocument => opt(alt((
