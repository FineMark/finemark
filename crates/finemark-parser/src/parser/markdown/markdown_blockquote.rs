...
pub(crate) fn markdown_blockquote_parser(parser_input: &mut ParserInput) -> Result<Element> {
    ...
    let marker_spans: Vec<Span> = raw_lines
        .iter()
        .filter_map(|line| line.marker_span.map(|(start, end)| Span::new(start, end)))
        .collect();
...
fn blockquote_lazy_continuation_line<'i>(
    ...
    // Policy: FineMark keeps blockquote lazy continuation permissive.
    // If indentation matches `content_indent`, any non-empty line remains part of
    // the quote even when it looks like another block starter. Those lines are
    // re-parsed inside the quote, not at the root level.
