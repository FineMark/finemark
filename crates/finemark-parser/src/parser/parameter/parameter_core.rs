...
fn parameter_parser<'i>(parser_input: &mut ParserInput<'i>) -> Result<(&'i str, Parameter<'i>)> {
...
fn parameter_list_parser<'i>(parser_input: &mut ParserInput<'i>) -> Result<Parameters<'i>> {
    ...
    separated(0.., parameter_parser, comma_separator)
        .map(|pairs: Vec<_>| pairs.into_iter().collect::<Parameters<'i>>())
        .parse_next(parser_input)
}

pub(crate) fn parameter_core_parser<'i>(
